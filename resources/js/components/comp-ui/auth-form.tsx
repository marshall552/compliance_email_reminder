...
const formCardVariants = {
    hidden: { opacity: 0, y: 50 }, // Start 50px to the right
    visible: {
        opacity: 1,
...
export default function AuthForm<T extends Record<string, any>>({
    ...
    return (
        <motion.div
            className="bg-[#F9F9F9] rounded-lg shadow-lg p-8 w-full max-w-md font-poppins" // Adopted styling from Register
            variants={formCardVariants}
            initial="hidden"
            animate="visible"
        >
            <h1 className="text-3xl font-extrabold mb-0 text-center font-poppins">
                {title}
            </h1>
            {subtitle && (
                <p className="text-sm text-gray-500 mb-6 text-center font-poppins font-extralight">
                    {subtitle}
                </p>
            )}
            <form className="space-y-4" onSubmit={onSubmit}>
                {inputs.map((input) => (
                    <div key={input.id}>
                        <div className="flex items-center">
                            <Label htmlFor={input.id} className="font-poppins">
                                {input.label}
                            </Label>
                            {input.id === 'password' && canResetPassword && (
                                <TextLink href={route('password.request')} className="ml-auto text-sm font-poppins">
                                    Forgot password?
                                </TextLink>
                            )}
                        </div>
                        <Input
                            ...
                            autoComplete={input.autoComplete}
                            tabIndex={input.tabIndex}
                            value={form.data[input.id] ?? ''}
                            onChange={(e) => form.setData(input.id as keyof T, e.target.value)}
                            className="mt-1 font-poppins"
                        />
                        <InputError message={errors[input.id]} className="font-poppins" />
                    </div>
                ))}
                {showRemember && remember !== undefined && setRemember && (
                    <div className="flex items-center space-x-3">
                        <Checkbox
                            ...
                            onClick={() => setRemember(!remember)}
                        />
                        <Label htmlFor="remember" className="font-poppins">
                            Remember me
                        </Label>
                    </div>
                )}
                <div className="flex justify-center">
                    <motion.button
                        type="submit"
                        className="w-full py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 font-open-sans font-bold"
                        whileHover={buttonHover}
                        disabled={processing}
                    >
                        {processing && <LoaderCircle className="h-4 w-4 animate-spin inline-block mr-2" />}
                ...
                <div className="flex justify-center">
                    <span className="text-sm text-gray-500 font-poppins">
                        {secondaryText}{' '}
                        <TextLink href={secondaryLink.href} className="text-blue-600 hover:underline font-poppins">
                            {secondaryLink.text}
                        </TextLink>
                    </span>
                </div>
